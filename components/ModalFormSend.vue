<template>
   ...
   <div class="modal-card" style="width: auto">
        <header class="modal-card-head">
            <p class="modal-card-title">ส่งหนังสือ</p>
        </header>
        <section class="modal-card-body">

            <b-field grouped>
                <b-field label="วันที่ส่ง *" expanded>
                    <b-datepicker
                        v-model="dateSend"
                        locale="th"
                ...
                <b-field label="เวลาส่ง *" expanded>
                    <b-timepicker
                        v-model="time_send"
...
export default {
    ...
    data() {
        return {
            ...
            dateSend: this.dateSendData,
            book_date: this.bookDateData,
            time_send: this.timeSendData,
        ...
        async save () {
            ...
            try {
                ...
                if(this.typeAction === 'new') {
                    result = await this.$axios.$post(`/api/v1/doc/send`,  this.dataPost)
                } else {
                    const id = String(this.idBook)
                    const ciphertext = CryptoJS.AES.encrypt(id, process.env.SECRET_KEY)
                    result = await this.$axios.$put(`/api/v1/doc/send/`+encodeURIComponent(ciphertext),  this.dataPost)
                }
