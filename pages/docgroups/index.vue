    ...
    <section class="section">
        <header class="docs-header">
            <b-breadcrumb align="is-left" size="is-medium">
                ...
                <b-breadcrumb-item tag='router-link' to="agencys">หมวดหนังสือ</b-breadcrumb-item>
            </b-breadcrumb>
        ...
        <div class="columns">
            <div class="column">
                <b-table
                    ...
                    <b-table-column v-slot="props" field="group" label="หมวดหนังสือ" sortable searchable>
                        {{ props.row.group }}
                    </b-table-column>
...
import ModalFormDocGroupVue from '../../components/ModalFormDocGroup.vue'
...
export default {
    ...
    methods: {
        async fetchData () {
            const result = await this.$axios.get('/api/v1/doc/group')
            this.groupData = result.data[0]
        },
        cardModal() {
            this.$buefy.modal.open({
                parent: this,
                component: ModalFormDocGroupVue,
                hasModalCard: true,
                fullScreen: false,
                canCancel: false,
                props: {
        ...
        async editItem(item) {
            try {
                ...
                const result  = await this.$axios.get('/api/v1/doc/group/'+encodeURIComponent(ciphertext))
                ...
                this.$buefy.modal.open({
                    ...
                    props: {
                        typeAction: 'edit',
                        idGroup: result.data[0].id,
                        groupName: result.data[0].group,
                        groupStatus: ((result.data[0].status === 'Y')?'true':'false')
                    },
                    events: {
                        'close':value => {
                            if(value === 'ok'){
        ...
        async deleteItem(item) {
            try {
                await this.$buefy.dialog.confirm({
                    title: 'ลบหมวดหนังสือ',
                    message: 'คุณต้องการลบหมวดหนังสือ ใช่ หรือ ไม่',
                    confirmText: 'ลบ',
                    cancelText: 'ยกเลิก',
                    type: 'is-danger',
                    hasIcon: true,
                    onConfirm: () => {
                        try {
                            const id = String(item.id)
                            const ciphertext = CryptoJS.AES.encrypt(id, process.env.SECRET_KEY)
                            this.$axios.delete('/api/v1/doc/group/'+encodeURIComponent(ciphertext))
